...
class BannerClaimableCounts(PackPrimitive[list[int]]):
    """
    ...
    def __repr__(self):
        ...
        return f"<{self.__class__.__qualname__}: {total} claimable banners>"
    ...
    def _validate(cls, value: list[int], **kwargs: Any) -> None:
        ...
        for item in value:
            if not FileProcessor.USHORT_MIN <= item <= FileProcessor.USHORT_MAX:
    ...
    def _write(cls, fp: FileProcessor, value: list[int], **kwargs: Any) -> None:
        ...
        for item in value:
            fp.write_ushort(item)


__all__ = ("BannerClaimableCounts",)
